 ...
 * 
 * This is a JPF specific version of a system class because we can't use the
 * real, platform JVM specific version (it's native all over the place, its
 * field structure isn't documented, most of its methods are private, hence we
 * can't even instantiate it properly).
 * 
 * Note that this class never gets seen by the real VM - it's for JPF's eyes
 * only.
 * 
 * For now, it's only a fragment to test the mechanism, and provide basic
 * class.getName() / Class.ForName() support (which is (almost) enough for Java
 * assertion support
 */
...
public final class Class<T> implements Serializable, GenericDeclaration, Type, AnnotatedElement {
  ...
  /**
   * this is the StaticArea ref of the class we refer to (so that we don't have
   * to convert to a Java String in the peer all the time)
   */
  private int cref;
  ...
  public native Field getDeclaredField (String fieldName) throws NoSuchFieldException, SecurityException;

  public native Field[] getDeclaredFields () throws SecurityException;

  public native Method getDeclaredMethod (String mthName, Class<?>... paramTypes) throws NoSuchMethodException, SecurityException;
  ...
  public Package getPackage () {
    ...
    if (idx >= 0) {
      ...
      Package pkg = new Package(pkgName, "spectitle", "specversion", "specvendor", "impltitle", "implversion", "implvendor", null, getClassLoader());
      return pkg;
  ...
  // <2do> we should move this to the native side, since Enum constants don't
  // change
  ...
  public native Constructor<T> getDeclaredConstructor (Class<?>... paramTypes) throws NoSuchMethodException, SecurityException;
