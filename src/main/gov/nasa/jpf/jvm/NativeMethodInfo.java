...
public class NativeMethodInfo extends MethodInfo {
  ...
  public Instruction executeNative (ThreadInfo ti) {
    ...
    /*** This part is added to use the jpf-nhandler project ***/
    if (mth == null) {
      System.out.println("*** Native method " + this.ci.getName() + "." + this.name + " is NULL");
    ...
    try {
      ...
      if (top == nativeFrame) { // no roundtrips, straight return
        ...
        if (env.isInvocationRepeated()) {
          ...
          return nativeFrame.getPC().getNext(); // that should be the
                                                // NATIVERETURN
        }

      } else {
        // direct calls from within the native method, i.e. nativeFrame is not
        // on top anymore, but its current instruction (invoke) will be
        // reexecuted
        // because DirectCallStackFrames don't advance the pc of the new top top
        // upon return
        return top.getPC();
      }
    ...
    // }
      ...
      return ti.createAndThrowException("java.lang.IllegalAccessException", "calling " + ci.getName() + '.' + getName());
    } catch (InvocationTargetException itx) {
      if (itx.getTargetException() instanceof UncaughtException) { // Native
                                                                   // methods
                                                                   // could
        throw (UncaughtException) itx.getTargetException();
      }

      // this will catch all exceptionHandlers thrown by the native method
      // execution
  ...
  /**
   * Get and convert the native method parameters off the ThreadInfo stack. Use
   * the MethodInfo parameter type info for this (not the reflect.Method type
   * array), or otherwise we won't have any type check
   */
